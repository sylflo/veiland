...
# The artifact under test: the locally-built .rpm from dist/ by default, or --
...
# pick a local build when a released one was asked for (or vice versa). The
# veiland-[0-9]* glob excludes the -debuginfo and -debugsource subpackages:
# only the main package is what a user installs.
...
if [[ "${1:-}" == "--release" ]]; then
    ...
    mkdir -p "$DIST_DIR"
    shopt -s nullglob
    cached=("$DIST_DIR"/veiland-[0-9]*.rpm)
    shopt -u nullglob
    if [[ ${#cached[@]} -eq 0 ]]; then
        echo ">> downloading the $RELEASE_TAG .rpm from the GitHub release"
        (cd "$REPO_ROOT" && gh release download "$RELEASE_TAG" \
            --pattern '*.rpm' --dir "$DIST_DIR")
    fi
    if [[ -f "$DISK" ]]; then
        echo ">> NOTE: an overlay disk already exists, and cloud-init only installs"
        echo ">> on FIRST boot -- the release .rpm will not be installed into it."
        echo ">> Run './fedora.sh --clean' first, then rerun with --release."
    fi
fi

shopt -s nullglob
rpms=("$DIST_DIR"/veiland-[0-9]*.rpm)
shopt -u nullglob
if [[ ${#rpms[@]} -eq 0 ]]; then
    if [[ -n "$RELEASE_TAG" ]]; then
        echo "!! no .rpm asset on release $RELEASE_TAG." >&2
    else
...
if [[ -n "$RELEASE_TAG" ]]; then
    echo ">> package under test: $RPM_NAME (release $RELEASE_TAG)"
else
    echo ">> package under test: $RPM_NAME (locally built)"
fi
