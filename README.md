...
Build-Depends — distro rustc packages can lag behind what the
dependency tree needs — so install one first
([rustup](https://rustup.rs) is the easy way), then:
...
**What a hostile plugin can still try inside the boundary, and how it's bounded.** Malformed messages get its socket closed and a fallback drawn for its region. Resource exhaustion is bounded by construction rather than by quotas: buffer dimensions are capped, every message is decoded from a fixed-size buffer, and the core retains at most one imported buffer per plugin — each new frame replaces the previous one, so a plugin flooding buffers costs the core some churn, never unbounded memory or file descriptors. (An explicit in-flight cap enforcing the frame-pacing protocol is planned for a future release as further protocol hardening.) The locker never blocks on a dead plugin. A plugin could draw a fake "unlocked" desktop inside its own region, which is why the password UI is painted by the core on top of all plugin output: a plugin can draw beneath it, never over it.
