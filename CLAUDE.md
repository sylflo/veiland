...
8. **M7 — Multi-monitor (partial):** per-output plugin instances, per-plugin output selection in config (`monitors = ["DP-1", "DP-2"]`), output identity carried on `Configure`, output **hotplug-out** handling. M6 shipped with a "every plugin on every output" shortcut; M7 is when each monitor gets its own independent instance of each plugin that opted into it. Done before reference plugins because writing the reference plugins against M6's shortcut would bake in assumptions a later multi-monitor pass would have to undo. **Hotplug-in (creating a lock surface for a newly-arrived monitor mid-lock) is deferred to M8** — we hit an SCTK/Hyprland `wl_output` rebinding interaction that crashes the host. See `docs/m7-plan.md` "Known limitations after step 5."
9. **M8 — Hotplug-in (the missing half of multi-monitor):** the security-critical other half of M7. A monitor plugged in mid-lock must get a lock surface and its plugins, without crashing the host. Blocks pre-1.0 ship because a non-locked monitor mid-session is a security hole. Reference plugins move to M9. See `docs/m8-plan.md`.
10. **M9 — Reference plugins:** clock, simple wallpaper, shader background. Three plugins to validate the API and seed the ecosystem.
