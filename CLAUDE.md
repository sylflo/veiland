...
12. **M11 — Reference plugins:** done. Four plugins land — `veiland-wallpaper` (one fixed image, PNG/JPEG via the `image` crate with `default-features = false` for CVE-surface reasons), `veiland-clock` (time + date labels via `veiland-text` and `chrono` strftime patterns), `veiland-particles` (slow upward drift of small dots, fixed shape, geometry-based with per-particle randomised cycle in 10-18s), and `veiland-vignette` (four-corner radial-gradient via a single fragment shader with `precision highp float` against Mesa banding). One host change rides along: a 30s periodic `Configure` tick so time-aware plugins stay current without each one reading the system clock — plugins remain pure functions of host events. `docs/examples/m11-shinkai.toml` composes all four plus two `veiland-label` instances into a Shinkai-mockup recreation at scene 1 ("Your Name"). `docs/plugin-api.md` extended with "Loading image assets" and "Procedural shader plugins" sections — the reference plugins are the real documentation. **Known limitations** (captured in `docs/improvements.md` and not blocking M11): per-plugin frame rate is bottlenecked at ~10-13 fps by the host's serialised buffer round-trip, so particles look choppy — real fix is the multi-buffer pool M5 set up infrastructure for; cosmic-text glyphs render visibly chunkier than Cairo's subpixel-AA path at small sizes; wallpaper decode is on the main thread and blocks the lock surface for ~5s on a 4K JPEG. All three are M12 candidates. Theme switching, slideshow rotation, animation timelines, particle variety, letter-spacing, italics, and font fallback config are also M12+.
