...
//! licence). Per-petal seeds (X, time offset, cycle, drift, spin) are
...
/// Default bundled petal asset, relative to the binary's working dir
/// (the repo root, when launched from `target/debug/...`).
const DEFAULT_PETAL_PATH: &str = "plugins/sakura/assets/petal.png";
...
struct Config {
    ...
    size_px: f32,
    /// Path to the petal PNG. Defaults to the bundled asset.
    #[serde(default = "default_petal_path")]
    petal_path: String,
}
...
fn default_petal_path() -> String {
    DEFAULT_PETAL_PATH.to_string()
}
...
/// Decode the petal PNG to RGBA8. Returns None (logged) on any failure —
/// a missing/bad asset must not crash the locker; render() then draws
/// nothing.
fn load_petal_rgba(path: &str) -> Option<(u32, u32, Vec<u8>)> {
    ...
    let img = match image::load_from_memory_with_format(&bytes, image::ImageFormat::Png) {
        ...
        Err(e) => {
            eprintln!(
                "veiland-{}: failed to decode petal PNG {:?}: {}",
                PLUGIN_NAME, path, e
            );
            return None;
        }
    ...
    eprintln!(
        "veiland-{}: loaded petal {:?} as {}x{} RGBA",
        PLUGIN_NAME, path, w, h
    );
...
/// 6 verts/petal.
unsafe fn build_gpu_state(petal_path: &str) -> GpuState {
    ...
    unsafe {
        ...
        let texture = match load_petal_rgba(petal_path) {
            Some((w, h, rgba)) => {
...
fn run() -> Result<(), PluginError> {
    ...
    eprintln!(
        "veiland-{}: config count={} color={:?} size_px={} petal_path={:?}",
        PLUGIN_NAME, config.count, config.color, config.size_px, config.petal_path
    );
    ...
    let gpu = unsafe { build_gpu_state(&config.petal_path) };
