...
//!
//! Sister plugins: `blue-box` (50% opaque), `green-box` (fully
...
const PLUGIN_NAME: &str = "red-box";

// Buffer is small because we render a solid colour — the host
// stretches it to whatever region the user configured.
const BUFFER_WIDTH: u32 = 64;
const BUFFER_HEIGHT: u32 = 64;

// (R, G, B, A) — straight (non-pre-multiplied) alpha per
// docs/protocol.md §12.1. Red at 50% opacity: where this plugin's
// region overlaps a lower-z plugin, you should see a visible blend
// (red-on-dim-blue produces a clear purple, not just "slightly
// pinker red"). 78% was the plan's first guess and was too subtle.
const COLOR_R: f32 = 1.0;
const COLOR_G: f32 = 0.0;
const COLOR_B: f32 = 0.0;
const COLOR_A: f32 = 0.5;
...
/// Compile a trivial "output solid colour" shader. No uniforms, no
/// varyings — the colour is baked into the fragment shader text at
/// startup so we don't need to push a uniform per frame.
...
fn run() -> Result<(), PluginError> {
    ...
    // Same buffer-lifecycle flags as the gradient plugin. See
    // plugins/gradient/src/main.rs for the longer comment.
